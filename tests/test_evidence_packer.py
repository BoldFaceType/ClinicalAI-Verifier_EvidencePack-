...
from evidence_packer.llm.evidence_extractor import EvidenceExtractionResponse
...
class MixedAIClient:
    def create(self, *args, **kwargs) -> EvidenceExtractionResponse:
        return EvidenceExtractionResponse(
            excerpts=[
                "Prior auth was approved on 2025-08-10.",
                "The patient completed six months of conservative therapy.",
            ]
        )
...
class EvidencePackerTests(unittest.TestCase):
    ...
    def test_run_packaging_records_ai_verification_audit(self) -> None:
        temp_path = make_temp_dir("evidence_ai_audit")
        try:
            claim_response_json = temp_path / "claimresponse_denied.json"
            notes_dir = temp_path / "clinical_notes"
            output_dir = temp_path / "out"
            ...
            (notes_dir / "note_1.txt").write_text(
                "Prior auth was approved on 2025-08-10. Referral number is documented.",
                encoding="utf-8",
            )
            summary = run_packaging(
                ...
                output_dir,
                use_ai=True,
                ai_client=MixedAIClient(),
            )

            self.assertEqual(summary["ai_assisted"], True)
            self.assertEqual(summary["verified_evidence_count"], 1)
            self.assertEqual(summary["rejected_evidence_count"], 1)
            packet = json.loads((output_dir / "packet.json").read_text(encoding="utf-8"))
            self.assertEqual(packet["ai_audit"]["verified_evidence_count"], 1)
            self.assertEqual(packet["ai_audit"]["rejected_evidence_count"], 1)
        finally:
            cleanup_temp_dir(temp_path)
